<%@ Page Language="C#" MasterPageFile="~/Essel.master" AutoEventWireup="true" CodeFile="frmDCABudget.aspx.cs"
    Inherits="Admin_frmDCABudget" EnableEventValidation="false" Title="Assign DCA Yearly Budget - Essel Projects Pvt. Ltd." %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    function validate()
    {
        var objs=new Array("<%=ddlDCAcode.ClientID %>","<%=ddlCCcode.ClientID %>","<%=ddlyear.ClientID %>","<%=txtBudget.ClientID %>");        
        if(!CheckInputs(objs))
        {
        return false;
        }
        ...
        {
            var sbud=document.getElementById("<%=lbldcabud.ClientID %>").innerHTML;
            var bud=document.getElementById("<%=lblccbud.ClientID %>").innerHTML;
            var nbud=document.getElementById("<%=txtBudget.ClientID %>").value;            
            if(bud=="Budget Not Assigned")
            {
                alert("CC budget not Assigned");
                return false;
            }
            else if(sbud!="Budget Not Assigned")
            {
                alert("Budget Already Assigned to this DCA");
                return false;
            }
            else if(Number(nbud)>Number(bud))
            {
                alert("Insufficient CC Budget");
                return false;
            }
        }
    }
                                ...
                                <tr>
                                    ...
                                    <td align="left">
                                        ...
                                        <cc1:CascadingDropDown ID="CascadingDropDown1" runat="server" TargetControlID="ddlDCAcode"
                                            ServicePath="cascadingDCA.asmx" Category="DCA" LoadingText="please Wait"
                                            ServiceMethod="ISdc" PromptText="Select DCA" ParentControlID="ddlCCcode">
                                        </cc1:CascadingDropDown>
                                        <span class="starSpan">*</span>
