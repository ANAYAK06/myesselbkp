...
using System.Data;
public partial class ITSummary : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if ((Request.QueryString["ITCode"] == null))
            {
    ...
    public void name()
    {
        da = new SqlDataAdapter("select it_name from it where it_code='" + Request.QueryString["ITCode"].ToString() + "'", con);
        da.Fill(ds, "name");
        ViewState["codename"] = ds.Tables["name"].Rows[0].ItemArray[0].ToString();
    }

    public void filldata()
    {
        da = new SqlDataAdapter("ITReport_sp", con);
        da.SelectCommand.CommandType = CommandType.StoredProcedure;
        da.SelectCommand.Parameters.AddWithValue("@ITCode", SqlDbType.VarChar).Value = Request.QueryString["ITCode"].ToString();
    ...
    protected void GridView1_RowCreated(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            Cell_Header2.Text = "IT :-" + Request.QueryString["ITCode"].ToString() + "  ( " + ViewState["codename"].ToString() + ") DETAIL VIEW";
            Cell_Header2.HorizontalAlign = HorizontalAlign.Center;
            Cell_Header2.ColumnSpan = 10;
    ...
    protected void btnExcel_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string s = "IT :-" + Request.QueryString["ITCode"].ToString() + "  ( " + ViewState["codename"].ToString() + ") DETAIL VIEW.xls";
    ...
    ////    Response.ClearContent();
    ////    Response.Buffer = true;
    ////    string s = "IT :-" + Request.QueryString["ITCode"].ToString() + "  ( " + ViewState["codename"].ToString() + ") DETAIL VIEW";
    ////    Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", s));
    ////    Response.ContentType = "application/ms-excel";
    ////    //StringWriter sw = new StringWriter();
    ////    //HtmlTextWriter htw = new HtmlTextWriter(sw);
